...
![Research pipeline overview](/assets/images/01_research_flow.svg "Figure 1. Research pipeline — from Beta distribution hypothesis rejection through v_t scale-dependency discovery to the final v3 algorithm.")
...
![Core algorithm pipeline](/assets/images/02_hadamard_lloyd_max_pipeline.svg "Figure 2. Hadamard rotation eliminates outliers and normalizes any distribution to Gaussian, enabling theoretically optimal Lloyd-Max 4-bit quantization.")

### AdamW–Error Feedback Incompatibility (Key Discovery)

![Error Feedback failure mechanism](/assets/images/06_ef_failure_mechanism.svg "Figure 3. SGD's linear update preserves EF guarantees; AdamW's nonlinear denominator $1/(\sqrt{v_t}+\epsilon)$ amplifies delayed noise, destroying momentum state.")
...
![v_t strategy scale dependency](/assets/images/09_vt_scale_dependency.svg "Figure 4. Variance freezing succeeds at ~1M parameters but diverges at 124M+ scale. Live v_t with error compensation is the only scale-invariant strategy.")
...
![TurboQuantAdamW v3 algorithm structure](/assets/images/08_v3_algorithm.svg "Figure 5. TurboQuantAdamW v3 — live v_t + momentum-level error compensation. Error never enters v_t path, preserving nonlinear denominator stability.")
...
![DDP communication pipeline](/assets/images/03_ddp_communication_pipeline.svg "Figure 6. Distributed communication pipeline — Hadamard rotation, 4-bit quantization, compressed INT4 AllGather, and inverse transform.")

### 4-bit Activation Memory Compression

![Activation compression architecture](/assets/images/07_activation_compression.svg "Figure 7. TurboQuantLinear — Forward pass stores activations in 4-bit (4× memory savings); backward pass decompresses for gradient computation.")
...
![Comprehensive experiment results from 1M to 355M parameters](/assets/images/11_comprehensive_results.svg "Figure 8. Full validation spectrum — from concept validation on M4 to GPU-scale experiments on 2× RTX 5000.")
...
![1,500-step convergence stability](/assets/images/10_long_convergence.svg "Figure 9. Loss gap remains bounded (0.0009–0.0024) across 1,500 steps — error compensation prevents accumulation.")
